...
for F in *.java; do
	...
	cat $F | sed "s/net.sourceforge.clonekeenplus/$AppFullName/" >> ../src/$F # | sed 's@$@ // THIS FILE IS AUTO-GENERATED@' >>
done
cd ../..
...
cat project/AndroidManifestTemplate.xml | \
	...
	sed "s^{{config.package}}^$AppFullName^g" | \
...
cat project/jni/SettingsTemplate.mk | \
	sed "s/APP_MODULES := .*/APP_MODULES := application sdl-$LibSdlVersion sdl_main jpeg png ogg flac vorbis freetype $CompiledLibraries/" | \
